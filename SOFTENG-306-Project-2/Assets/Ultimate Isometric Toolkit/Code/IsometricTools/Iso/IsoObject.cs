...
public class IsoObject : MonoBehaviour, IComparable<IsoObject> {
	...
	public int CompareTo(IsoObject other) {
		...
		if(other.bounds.min.z *(1+ intersectionTolerence) < bounds.max.z && other.bounds.max.x > bounds.min.x * (1+ intersectionTolerence) && other.bounds.max.y > bounds.min.y * (1+ intersectionTolerence)) {
