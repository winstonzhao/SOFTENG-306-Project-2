...
public class TurnbasedIsoObjectController : AbstractIsoObjectController
{
    ...
    void forward()
    {
        if (canMoveForward())
        {
            ...
            moveTo(newPos, (x) => EasingFunctions.QuadEaseOut(x, 0, 1, forwardSpeed), () => { inAction = false; }, 0,
                forwardSpeed);
        }
    }
    ...
    private void jumpUp()
    {
        var finalNewPos = posInMapToWorldPos(positionInMap + Isometric.vectorToIsoDirection(currentForwardDirection) +
                                             Isometric.vectorToIsoDirection(IsoDirection.Up));
        var tempNewPos = posInMapToWorldPos(positionInMap +
                                            Isometric.vectorToIsoDirection(currentForwardDirection) * 2 / 3f +
                                            Isometric.vectorToIsoDirection(IsoDirection.Up) * 4 / 3f);
        inAction = true;
        moveTo(tempNewPos, (x) => EasingFunctions.Linear(x, 0, 1, forwardSpeed * 2 / 3f), () =>
            {
                moveTo(finalNewPos, (x) => EasingFunctions.Linear(x, 0, 1, forwardSpeed * 1 / 3f), () =>
                    {
                        positionInMap = positionInMap + Isometric.vectorToIsoDirection(currentForwardDirection) +
                                        Isometric.vectorToIsoDirection(IsoDirection.Up);
                        inAction = false;
                    }, 0, forwardSpeed * 1 / 3f);
            }, 0, forwardSpeed * 2 / 3f);
    ...
    {
        var finalNewPos = posInMapToWorldPos(positionInMap + Isometric.vectorToIsoDirection(currentForwardDirection) +
                                             Isometric.vectorToIsoDirection(IsoDirection.Down));
        var tempNewPos = posInMapToWorldPos(positionInMap +
                                            Isometric.vectorToIsoDirection(currentForwardDirection) * 1 / 2f +
                                            Isometric.vectorToIsoDirection(IsoDirection.Up));
        inAction = true;
    ...
    {
        return map.getTile(positionInMap + Isometric.vectorToIsoDirection(currentForwardDirection) +
                           Isometric.vectorToIsoDirection(IsoDirection.Down) * 2);
    }
    ...
    void OnDrawGizmos()
    {
        ...
        Gizmos.DrawRay(transform.position,
            Isometric.isoProjection(Isometric.vectorToIsoDirection(currentForwardDirection)));
    }
}
