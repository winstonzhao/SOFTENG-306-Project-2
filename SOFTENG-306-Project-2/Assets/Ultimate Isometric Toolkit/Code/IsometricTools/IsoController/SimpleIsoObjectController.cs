...
public class SimpleIsoObjectController : AbstractIsoObjectController
{
    ...
    new void Update()
    {
        translate(Isometric.vectorToIsoDirection(IsoDirection.North) * Input.GetAxis("Vertical") * Time.deltaTime *
                  speed);
        translate(Isometric.vectorToIsoDirection(IsoDirection.East) * Input.GetAxis("Horizontal") * Time.deltaTime *
                  speed);
    }
