...
using Ultimate_Isometric_Toolkit;
using UnityEditor;
using UnityEngine;
...
namespace Ultimate_Isometric_Toolkit.Editor
{
    ...
    public class IsoTransformEditor : UnityEditor.Editor
    {
        ...
        private void UpdateHandlePos()
        {
            ...
            _isoHandlePos = _selectedObjects.Aggregate(Vector3.zero, (runningSum, t) => runningSum + t.Position) /
        ...
        private void XAxisHandle()
        {
            ...
            var deltaMovement = Handles.Slider(screenHandlePos, Isometric.IsoToUnitySpace(Vector3.right)) -
                                screenHandlePos;
        ...
        private void YAxisHandle()
        {
            ...
            var screenHandlePos = Isometric.IsoToUnitySpace(_isoHandlePos);
            var deltaMovement = Handles.Slider(screenHandlePos, Isometric.IsoToUnitySpace(Vector3.up)) -
                                screenHandlePos;
            if (!(Mathf.Abs(deltaMovement.y) > Mathf.Epsilon))
                return;
        ...
        private void ZAxisHandle()
        {
            ...
            var screenHandlePos = Isometric.IsoToUnitySpace(_isoHandlePos);
            var deltaMovement = Handles.Slider(screenHandlePos, Isometric.IsoToUnitySpace(Vector3.forward)) -
                                screenHandlePos;
            if (!(Mathf.Abs(deltaMovement.y) > Mathf.Epsilon))
                return;
        ...
        private void ApplyZAxisChanges(float delta)
        {
            ...
            foreach (var t in _selectedObjects)
            {
                ...
                if (_instance != null && IsoSnapping.DoSnap)
                    newZ = (float) (Math.Round(newZ * 1 / IsoSnapping.SnappingVector.z, MidpointRounding.AwayFromZero) *
                                    IsoSnapping.SnappingVector.z);
        ...
        private void ApplyXAxisChanges(float delta)
        {
            ...
            foreach (var t in _selectedObjects)
            {
                ...
                if (_instance != null && IsoSnapping.DoSnap)
                    newX = (float) (Math.Round(newX * 1 / IsoSnapping.SnappingVector.x, MidpointRounding.AwayFromZero) *
                                    IsoSnapping.SnappingVector.x);
        ...
        private void ApplyYAxisChanges(float delta)
        {
            ...
            foreach (var t in _selectedObjects)
            {
                ...
                if (_instance != null && IsoSnapping.DoSnap)
                    newY = (float) (Math.Round(newY * 1 / IsoSnapping.SnappingVector.y, MidpointRounding.AwayFromZero) *
                                    IsoSnapping.SnappingVector.y);
