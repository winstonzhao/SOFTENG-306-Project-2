...
namespace Ultimate_Isometric_Toolkit.Scripts.Utils
{
    ...
    {
        [SerializeField] private static Matrix4x4 _isoMatrix = Matrix4x4.identity;

        public static Matrix4x4 IsoMatrix
        {
            get { return _isoMatrix; }
        ...
        /// <param name="yRot"></param>
        /// <returns></returns>
        public static Quaternion GetProjectionQuaternion(Projection projection, float xRot, float yRot)
        {
        ...
        public static Vector3 UnityToIsoSpace(Vector2 xyPos, Vector3 planeNormalVector, float offset)
        {
            ...
            var isoRay = new Ray(matrixInverse.MultiplyPoint(xyPos),
                matrixInverse.MultiplyPoint(new Vector3(0, 0,
                    1))); //create a ray that points in the camera direction and has 
        ...
        public Vector3 ScreenToIsoPoint(Vector2 screenSpacePoint, float zOffset = 0f, Camera cam = null)
        {
            if (cam == null)
                cam = Camera.main;
            var worldPos =
                cam.ScreenToWorldPoint(new Vector3(screenSpacePoint.x, screenSpacePoint.y, Camera.main.nearClipPlane));
