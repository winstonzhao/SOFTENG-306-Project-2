...
namespace Ultimate_Isometric_Toolkit.Scripts.Utils
{
    public static class GizmosExtension
    {
        ...
        public static void DrawIsoArrow(Vector3 from, Vector3 to, float arrowHeadLength = 0.25f,
        ...
        public static void DrawIsoWireSphere(Vector3 position, float radius, float angle)
        {
            ...
            for (var i = 1; i < 37; i++)
            {
                var new_x = new Vector3(position.x, position.y + radius * Mathf.Sin(angle * i * Mathf.Deg2Rad),
                    position.z + radius * Mathf.Cos(angle * i * Mathf.Deg2Rad));
                var new_y = new Vector3(position.x + radius * Mathf.Cos(angle * i * Mathf.Deg2Rad), position.y,
                    position.z + radius * Mathf.Sin(angle * i * Mathf.Deg2Rad));
                var new_z = new Vector3(position.x + radius * Mathf.Cos(angle * i * Mathf.Deg2Rad),
